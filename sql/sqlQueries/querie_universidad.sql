...
SELECT nombre, apellido1 FROM persona WHERE tipo = "alumno" AND YEAR(fecha_nacimiento) = 1999;
...
SELECT nombre, apellido1, nif FROM persona WHERE nif LIKE '%K' AND telefono IS NULL AND tipo = 'profesor';
...
SELECT persona.apellido1, persona.apellido2, persona.nombre, departamento.nombre FROM persona
    ...
    WHERE persona.tipo ='profesor' ORDER BY persona.apellido1, persona.apellido2, persona.nombre;
/* FROM persona p JOIN profesor pr ON p.id = pr.id_profesor JOIN departamento d ON d.id = pr.id_departamento ORDER BY apellido1, apellido2, p.nombre; */
...
SELECT DISTINCT persona.nombre, persona.apellido1 FROM persona
...
SELECT persona.apellido1, persona.nombre FROM persona
	RIGHT JOIN 	profesor ON persona.id = profesor.id_profesor
...
SELECT COUNT(nombre) FROM persona WHERE YEAR(fecha_nacimiento) = 1999 AND tipo = 'alumno';
...
SELECT COUNT(persona.nombre), departamento.nombre FROM departamento 
    ...
    WHERE persona.tipo ='profesor' AND profesor.id_departamento
    GROUP BY departamento.nombre
    ORDER BY COUNT(persona.nombre) DESC;
...
SELECT COUNT(*), departamento.nombre FROM departamento 
	INNER JOIN profesor ON departamento.id = profesor.id_departamento
    INNER JOIN persona ON profesor.id_profesor = persona.id
...
-- Devuelve el listado con el nombrre de todos los grados existentes en la base de  datos i rl numero de asignaturas que tiene cada uno de los grados  que tengan mas de 40 asignaturas asociadas
...
-- Devuelve el listado  que muestre el nombre de los grados  y sume el numero total de creditos que hay en casa asignatura i la suma de los creditos de todas las  asignaturas que hay de este tipo
SELECT g.nombre, a.tipo, SUM(a.creditos) FROM grado g JOIN asignatura a ON g.id = a.id_grado GROUP BY g.nombre, a.tipo;

-- Devuelve listado muestre cuantos alumnos matriculados en alguna asignatura en casa uno de los curso escolares. El resultado mostrará dos columnas, una acon el año de inicio y la otra numero alunmos matriculados
...
--Devuelve listado con el numero asignaturas imparte cada profesor. Tendra que tener  en cuenta  profesores que no impartan asignaturas. Resultado mostrará 5 columnas id, nombre, primer apellido, segundo apellido y numero asignatura Y estara ordenado de mayor a menos por ell numero de asignaturas
...
--devuelve los datos del alumno mas joven
SELECT * FROM persona WHERE tipo = 'alumno' ORDER BY fecha_nacimiento DESC LIMIT 1

-- Devuelve listado con los profesores que tienen un departamento asociado i que no imparten ninguna asignatura
